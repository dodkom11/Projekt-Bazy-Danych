...
BEGIN 
          ...
          INSERT INTO PRODUKT(DOSTAWCA_ID, KATEGORIA_ID, PRODUCENT, NUMER_KATALOGOWY, MODEL, CENA, SZTUK_NA_MAGAZYNIE, OPIS) VALUES (VDOSTAWCA_ID, VKATEGORIA_ID, VPRODUCENT, VNUMER_KATALOGOWY, VMODEL, VCENA, VSZTUK_NA_MAGAZYNIE, VOPIS);
...
BEGIN
  ...
  MY_QUERY := q'[SELECT KOSZYK.PRODUKT_ID, PRODUKT.PRODUCENT, PRODUKT.MODEL,  PRODUKT.NUMER_KATALOGOWY, PRODUKT.CENA, KOSZYK.ILOSC_SZTUK, (KOSZYK.ILOSC_SZTUK*PRODUKT.CENA) as ILOCZYN FROM KOSZYK INNER JOIN PRODUKT ON KOSZYK.PRODUKT_ID = PRODUKT.PRODUKT_ID WHERE KOSZYK.KONTO_ID =']' || KONT_ID || q'[']';
...
BEGIN
  ...
  MY_QUERY := q'[SELECT DISTINCT Z.ZAMOWIENIE_ID, Z.KONTO_ID, K.NAZWA_FIRMY, Z.DATA_ZALOZENIA_ZAMOWIENIA ,Z.DATA_PRZYJECIA_ZAMOWIENIA, Z.DATA_REALIZACJI_ZAMOWIENIA, Z.ZAMOWIENIE_ZAAKCEPTOWANE, Z.ZAPLACONO, Z.ZREALIZOWANO, Z.KOSZT_ZAMOWIENIA, Z.METODA_PLATNOSCI, Z.DATA_WYSYLKI, Z.DOKUMENT_SPRZEDAZY FROM ZAMOWIENIE Z, KURIER K WHERE Z.KURIER_ID = K.KURIER_ID]';
